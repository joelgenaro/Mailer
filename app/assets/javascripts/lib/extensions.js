...
$(function() {
  ...
  if (window.adminMailPlanInfoPath) {
    $('#mail_postal_mail_mail_recipient_id').on('change', function(e) {
      var userId = e.target.value;
      fetchMailPlanInfo(userId);
    });
    $(document).ready(function() {
      var userId = $('#mail_postal_mail_mail_recipient_id').val();
      if (userId) {
        fetchMailPlanInfo(userId);
      }
    });
  ...
  function fetchMailPlanInfo(userId) {
    var url = window.adminMailPlanInfoPath.replace(':id', userId);
    ...
    $.ajax({
      ...
      if (data.error !=null)
        { var innerHtml = data.error  }
      else
        { 
          var innerHtml = '<row><table> \
              <thead> \
                <tr> \
              ...
              <tbody> \
                ...
                <tr> \
                  ...
                  <td>'+ data.mail_translation_summary +'</td> \
                </tr> \
              <tbody> </row>\
            <table></panel>';
      }
